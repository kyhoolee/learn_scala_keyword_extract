...
/**
  ...
  */
class EntityExtractorService @Inject() (articleRepository: ArticleRepository,
										...
										entityGeonameRepository: EntityGeonameRepository) extends Logging {
	...
	private def getEntityCandidatesWithCapital(content: String) = Future {
	...
	private def combinePhrases(source: List[String], n: Int, takeRight: Boolean): List[List[String]] = {
		n match {
			case 1 => List(source)
			case _ => {
				if (takeRight)
					List(source) ::: combinePhrases(source.takeRight(source.length - 1), n -1, takeRight)
				else
					List(source) ::: combinePhrases(source.take(source.length - 1), n -1, takeRight)
			}
		}
	}

	private def breakLongPhrases(longWord: String) = Future {
		...
		val len = wordParts.length

		val words = wordParts.toList
		val entitiesToCheck = combinePhrases(words, words.length, true).flatMap { ss =>
			combinePhrases(ss, ss.length, false).map { ss2 =>
				ss2.mkString(" ") -> (words.length - ss.length)
			}
		}.toMap
	...
	private def mergeSynonym(entityCandidates: Map[String, TaggedEntity]) = Future {
	...
	private def cleanAndExtractCandidates(rawContent: String) = {
		val cleanedContent = tagPattern.replaceAllIn(rawContent, "\n")

		getEntityCandidatesWithCapital(cleanedContent).map(_.map { entityCandidate =>
	...
	private def filterAndBreakLongPhrases(entityCandidates: Map[String, TaggedEntity]) = Future {
		val candidates = mutable.Map.empty[String, TaggedEntity]
			...
			breakLongPhrases(entityCandidate._1).foreach { smallerPhrases =>
				if (smallerPhrases.size > 1) {
					...
					smallerPhrases.foreach { taggedEntity =>
						if (candidates.contains(taggedEntity.name)) {
							// if it is already exists as candidate, then increase its frequency
							candidates(taggedEntity.name).addAlias(taggedEntity.name, taggedEntity.occFreq)
						} else {
							// if it does not exists yet, then add new entry as candidate
							candidates += (taggedEntity.name -> taggedEntity)
						}
					}

					candidates -= entityCandidate._1
				}
	...
	private def weightenEntities(entityCandidates: Map[String, TaggedEntity]) = Future {
		// Step #7 weighten entity using its alias(es)
		val removeCandidates = mutable.ListBuffer.empty[String]
...
//						entityCandidates - entity1Name
						debug(s"add ${entity1Name} to removeCandidate")
						removeCandidates += entity1Name
					}
				}
			}
		}

		entityCandidates.filterNot(entity => removeCandidates.contains(entity._1.toLowerCase))
	}

	private def divideMatchUnmatch(entityCandidates: Map[String, TaggedEntity]) = Future {
		val matches = mutable.ListBuffer.empty[Entity]
		val unmatches = mutable.ListBuffer.empty[Entity]
		...
		entityCandidates.filterNot(ent => geonameList.contains(ent._1.toLowerCase)).foreach { entityCandidate =>
			if (entityCandidate._2.occFreq > 1)
				matches += Entity(entityCandidate._2.name, entityCandidate._2.occFreq, Option(entityCandidate._2.entityType.toInt))
			else
	...
	def extractEntitiesById(rawContent: String) = for {
			entityCandidates <- cleanAndExtractCandidates(rawContent)

			entityCandidates <- filterAndBreakLongPhrases(entityCandidates)
			...
			entityCandidates <- updateEntityType(matchTaggedEntities, entityCandidates)

			entityCandidates <- weightenEntities(entityCandidates)
