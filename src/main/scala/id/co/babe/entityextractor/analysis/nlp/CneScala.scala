...
class CneScala {
	...
	def genGroupCan(text: String) = for {
		...
		countMap <- countCan(combination.toMap.keys.toSeq, text)
		groupMap <- groupCan(longonly, countMap)
		matched <- redirectCandidate(groupMap)
	} yield buildResult(matched, combination, groupMap)

	def processCapitalized(text: String) = Future {
		val sents:Seq[String] = TextParser.sentenize(text)
			...
			words.zipWithIndex.map { word =>
				if(next == word._2) {
					if(checkCapital(word._1)){
						...
						while (iterate < words.length && checkCapitalorNumeric(words(iterate))) {
							candidate += " " + words(iterate)
							iterate += 1
							count += 1
						}
	...
	def checkCapital(word: String) = {
		if (word != null && word.length() > 0) {
			word.exists(f => Character.isUpperCase(f))
		} else false
	}

	def checkCapitalorNumeric(word: String) = {
	...
	def postProcess(can: String) = {
		can.split(" ").filter(w => !checkWeb(w)).mkString(" ")
	}

	def checkWeb(w: String) = {
	...
	def textReplace(text: String) = {
		text.replace(":", " :").replace("'s", " 's")
	}

	def countComb(com: String, text: String) = {
		var lastIndex: Int = 0
		var count: Int = 0
		while (lastIndex != -1) {
	...
	def filterShort(candidates: Seq[(String, Int)]) = Future {
	...
	def countCan(longCandidates: Seq[String], text: String) = Future {
		longCandidates.map{lc =>
			(lc, countDecay(lc, text))
		}
	}

	def countDecay(can: String, text: String) = {
		var lastIndex: Int = 0
		var count: Double = 0
		while (lastIndex != -1) {
	...
	def groupCan(candidates: Seq[String], countSet: Seq[(String, Double)]) = Future {
		candidates.map{can =>
	...
	def redirectCandidate(input: Seq[(String, Double)]) = Future {
		...
		rootCandidate.map(m => (rootCandidate.getOrElse(m._1,""), redirectScore.getOrElse(m._1, 0D))).toSeq
	}
	...
	def getMatchedEntity(cans: Seq[(String, Double)], counts: Seq[(String, Int)]) = {
		val cMap = counts.toMap
		cans.map{c => new JEntity(c._1, cMap.getOrElse(c._1, 0), c._2, 0)}
	}

	def getUnmatchedEntity(counts: Seq[(String, Int)], matched: Seq[String]) = {
